...
namespace AggregatorBDC.Classes
{
    public class AggregatorBDC : IAggregatorBDC
    {
        ...
        private string userServiceURL = Environment.GetEnvironmentVariable("USERSERVICE_URL") != null ? Environment.GetEnvironmentVariable("USERSERVICE_URL") : "http://stupefied_curie:80";
        private string orderServiceURL = Environment.GetEnvironmentVariable("ORDERSERVICE_URL") != null ? Environment.GetEnvironmentVariable("ORDERSERVICE_URL") : "http://adoring_goldstine:80";

        public AggregatorBDC() {}

        public AggregatorDTO GetOrderDetails(int id)
        {
            UserDTO userDetailsReceived = GetUserData().GetAwaiter().GetResult();
        ...
        async Task<UserDTO> GetUserData()
        {
            using (HttpClient client = new HttpClient())
            using (HttpResponseMessage res = await client.GetAsync(userServiceURL + "/api/user/1"))
